...
class MentionMentionCNNDecoder(object):
    ...
    def decode(self, model, mention_docs):
        ...
        for mentions in mention_docs:
            ...
            embs, ebfts = [], []
            for m in mentions:
                embs.append([eb.reshape(1, 1, neb, model.embdim) for eb, neb in zip(m.embedding, model.nb_embs)])
                ebfts.append(m.feature.reshape(1, model.embftdim))

            for c_idx, curr in enumerate(mentions[1:], 1):
                curr_emb, curr_ebft = embs[c_idx], ebfts[c_idx]

                target = None
                for p_idx, prev in reversed(zip(range(c_idx), mentions[:c_idx])):
                    prev_emb, prev_ebft = embs[p_idx], ebfts[p_idx]
